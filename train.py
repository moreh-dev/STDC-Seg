...
def parse_args():
    ...
    parse.add_argument(
        '--dspath',
        type=str,
        default='/home/junhyeok/sbox/STDC/data'
    )
    parse.add_argument(
        '--local_rank',
        dest='local_rank',
    ...
    parse.add_argument(
        '--args.n_workers_train',
        dest='args.n_workers_train',
        type=int,
        default=8,
    )
    parse.add_argument(
        '--args.n_workers_val',
        dest='args.n_workers_val',
        type=int,
        default=0,
    )
    parse.add_argument(
        '--args.n_img_per_gpu',
        dest='args.n_img_per_gpu',
        type=int,
        default=16,
    )
    parse.add_argument(
    ...
    parse.add_argument(
        '--args.use_boundary_2',
        dest='args.use_boundary_2',
        type=str2bool,
        default=False,
    )
    parse.add_argument(
        '--args.use_boundary_8',
        dest='args.use_boundary_8',
        type=str2bool,
        default=False,
    )
    parse.add_argument(
...
def evaluate(model, args, epoch):
    ...
    valset = TextDataset(src_fname=os.path.join(args.dataset,
    ...
    for nbatch, (src, indices) in enumerate(val_data_loader, 1):
        pass
...
def train():
    ...
    # setup_logger(args.respath)warning
    ## dataset
    n_classes = 19
    mode = args.mode
    ...
    dl = DataLoader(
        ds,
        batch_size=args.n_img_per_gpu,
        shuffle=False,
        num_workers=args.n_workers_train,
        pin_memory=False,
        drop_last=True)
    ...
    dlval = DataLoader(
        ...
        batch_size=2,
        shuffle=False,
        num_workers=args.n_workers_val,
        drop_last=False)
    ...
    score_thres = 0.7
    n_min = args.n_img_per_gpu * cropsize[0] * cropsize[1] // 16
    ...
    for it in range(max_iter):
        for nbatch, (im, lb) in dl:
            ...
            if args.use_boundary_2 and args.use_boundary_8 and args.use_boundary_8:
                out, out16, out32, detail2, detail4, detail8 = net(im)

            if (not args.use_boundary_2) and args.use_boundary_8 and args.use_boundary_8:
                out, out16, out32, detail4, detail8 = net(im)

            if (not args.use_boundary_2) and (not args.use_boundary_8) and args.use_boundary_8:
                out, out16, out32, detail8 = net(im)

            if (not args.use_boundary_2) and (not args.use_boundary_8) and (
                    not args.use_boundary_8):
            ...
            do_log, processed_iters = log_this_iter(nbatch)
            if do_log:
                ...
                tokens_per_sec = len_toks / duration
                iters_left = iters_per_eval - nbatch

                logger.info(
                    f'EVAL_STEP | '
                    f'Iteration: {nbatch:>5}/{iters_per_eval:>5} | '
                    f'Throughput: {tokens_per_sec:>5} toks/s | '
                    f'Duration: {duration:>5.3f} s | '
                    # f'Loss: {val_loss_per_token:>5.3f} | '
                    f'Estimated Time Remaining: {iters_left / iters_per_sec:.0f} s'
                )
                log_interval_time = time.perf_counter()
                len_toks = 0
